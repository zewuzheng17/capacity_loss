...
from tianshou.env import ShmemVectorEnv, VectorEnvNormObs, VectorEnvVecObs
...
def make_mujoco_env(task, seed, training_num, test_num, obs_norm, dmc_control=False):
    ...
    if obs_norm:
        ...
        test_envs = VectorEnvNormObs(test_envs)
        test_envs.set_obs_rms(train_envs.get_obs_rms())
    if dmc_control:
        print("using dmc environment")
        env = VectorEnvVecObs(env)
        train_envs = VectorEnvVecObs(train_envs)
        test_envs = VectorEnvVecObs(test_envs)
